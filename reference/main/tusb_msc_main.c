...
/* DESCRIPTION:
 * This code makes ESP32 based device recognizable by USB-hosts as a USB Mass Storage Device.
 * It either allows the embedded application to access the partition or Host PC accesses the partition over USB MSC.
 * They can't be allowed to access the partition at the same time.
 * For different scenarios and behaviour, refer to the project README.
 */
...
// Include common header first, then MSC header for constants BEFORE tinyusb.h
...
static const char *TAG = "msc_main";
static esp_console_repl_t *msc_repl = NULL;
...
static uint8_t const msc_fs_configuration_desc[] = {
    ...
    // Using explicit enum values: MSC_SUBCLASS_SCSI (6) and MSC_PROTOCOL_BOT (0x50)
...
const esp_console_cmd_t msc_cmds[] = {
    {
        .command = "read",
...
static int console_write(int argc, char **argv)
{
    ...
    if (!fd) {
        ...
        fprintf(fd, "The ESP32 chip will be recognised by host (PC) as Mass Storage Device.\n");
        fprintf(fd, "Upon connection to USB host (PC), the application will initialize the storage module and then the storage will be seen as removable device on PC.\n");
        fclose(fd);
    }
    return 0;
}
...
static esp_err_t storage_init_sdmmc(sdmmc_card_t **card)
{
    ...
    // Note: for fixed frequency of 10MHz, use host.max_freq_khz = 10000;
    ...
    // connected on the bus. This is for debug purposes only.
...
/**
 * @brief Initialize and run MSC device mode
 */
void msc_device_init(void)
{
    ...
    ESP_ERROR_CHECK(esp_console_new_repl_uart(&hw_config, &repl_config, &msc_repl));
    ...
    ESP_ERROR_CHECK(esp_console_new_repl_usb_serial_jtag(&hw_config, &repl_config, &msc_repl));

#else
#error Unsupported console type
#endif

    for (int count = 0; count < sizeof(msc_cmds) / sizeof(esp_console_cmd_t); count++) {
        ESP_ERROR_CHECK(esp_console_cmd_register(&msc_cmds[count]));
    }

    ESP_ERROR_CHECK(esp_console_start_repl(msc_repl));

    xSemaphoreTake(_wait_console_smp, portMAX_DELAY);
    ESP_ERROR_CHECK(esp_console_stop_repl(msc_repl));

...
#if defined COMPILE_FOR_ESP32
// OpenSerialPort and CloseSerialPort are implemented in esp32_serial_adapter.c
...
#if defined COMPILE_FOR_ESP32
    // Implementation in esp32_serial_adapter.c
    extern void SendComPortBlock_impl(ISP_ENVIRONMENT *IspEnvironment, const void *s, size_t n);
    SendComPortBlock_impl(IspEnvironment, s, n);
#elif defined COMPILE_FOR_WINDOWS || defined COMPILE_FOR_CYGWIN
...
*/
#if !defined INTEGRATED_IN_WIN_APP && !defined COMPILE_FOR_ESP32
    ...
    if (argc >= 5)
    {
        for (i = 1; i < argc - 3; i++)
        {
            ...
            if(strnicmp(argv[i],"-debug", 6) == 0)
            {
                char* num;
                num = argv[i] + 6;
                while(*num && isdigit((unsigned char)*num) == 0) num++;
                if(isdigit((unsigned char)*num) != 0) debug_level = atoi( num);
                else debug_level = 4;
...
#endif
        default:
            DebugPrintf(1, "Unknown microcontroller type\n");
            downloadResult = -1;
            break;
        }
